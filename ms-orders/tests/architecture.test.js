...
describe("Orders service architecture", () => {
  ...
  it("keeps domain and application independent from Express and MSSQL", () => {
    expectNoForbiddenDependency(files, /features\/orders\/(domain|application)\//, [
      /require\(["']express["']\)/,
      /require\(["']mssql["']\)/,
      /swagger-ui-express/,
      /swagger-jsdoc/
  ...
  it("keeps presentation away from direct database access", () => {
    expectNoForbiddenDependency(files, /features\/orders\/presentation\//, [
      /require\(["']mssql["']\)/,
      /\.query\(/
    ]);
  });

  it("keeps MSSQL usage inside infrastructure or config", () => {
    const violations = files
      .filter((file) => /require\(["']mssql["']\)/.test(file.content))
